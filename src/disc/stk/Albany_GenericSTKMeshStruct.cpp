...
void GenericSTKMeshStruct::initializeSideSetMeshStructs (const Teuchos::RCP<const Teuchos_Comm>& comm)
{
  if (params->isSublist ("Side Set Discretizations")) {
    ...
    for (int i(0); i<sideSets.size(); ++i) {
      ...
      stk::mesh::put_field(*side_to_cell_map, this->sideSetMeshStructs[ss_name]->metaData->universal_part(), 1, nullptr);
#ifdef ALBANY_SEACAS
      ...
      stk::mesh::put_field(*side_nodes_ids, this->sideSetMeshStructs[ss_name]->metaData->universal_part(), num_nodes, nullptr);
#ifdef ALBANY_SEACAS
